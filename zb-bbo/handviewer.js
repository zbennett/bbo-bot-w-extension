...
setTimeout(() => { 
	...
	if (!app.titleFixed) {
		...
		const bstr = document.getElementsByClassName('vulInnerDivStyle')[0].innerText;
...
document.addEventListener('keydown', (event) => {
	...
	else if (code === 'KeyN') { keycatch(code); toggleNameDisplay(); }
...
(err) => {
	console.error('BBO Helper: handviewer.js: Failed to add keydown event listener: ', err);
});
...
async function kickOffDD() {
	...
	const sleepTime = 5, maxTries = 100;
	for (let itry=1; itry<=maxTries; itry++) {
		await sleep(sleepTime); // mS
		if (app.prefLoaded) {
			console.info('BBO Helper: PREF seen to be loaded after %d mS', 
				sleepTime * itry );
			break;
		}

		if (itry === maxTries) {
			console.error('Unable to load PREF after %d mS. Using defaults', 
				maxTries * sleepTime);
		}
	}

	
	if (pref.hvDoubleDummyMode === 'off') { return; }
	...
	console.warn('BBO Helper: kickOffDD() was not able to parse a deal from ', 
		window.location);
}
...
function linFromMyhand(myhand) {
	...
	const url = 'https://webutil.bridgebase.com/v2/mh_handxml.php?id=' + myhand;
	
	fetchWithTimeout(url, {timeout: 5000} ).then(fetchSuccess, fetchError);

	
	function fetchError(err) {
		console.error('BBO Helper: myhand to LIN HTTP query failed for URL', url, 
			'Fetch erorr was', err);
	}
	...
	async function fetchSuccess(response) {
		...
		if ( response.status !== 200) {
			console.error('BBO Helper: myhand to LIN HTTP query failed for', URL,
				'with HTTP code', response.status);
			return;
		}
		...
		if ( err !== "0" ) {
			console.error('BBO Helper: myhand to LIN query returned BBO err', err, 
				'for URL', url);
			return;			
		}
		...
		// This works better.
		const boardMatches = app.lin.match( /(?<=ah\|Board )\d+/ );
		const boardstr = boardMatches === null ? '' : 'Board ' + boardMatches[0] + ' - ';
		
		app.datestr = strDate + ' ' + strLocalHHMM;
		const doctitle = boardstr + app.datestr + ' Session - BBO Handviewer';
...
function ddFromHands(hd, p) {
	...
	let vul = p.get('v');
	if (vul !== null) { vul = vul.toLowerCase(); }
...
async function doubleDummyCallback(d, dd) {
	...
	const sleepTime = 20, maxTries = 100;
	for (let itry = 0; itry<maxTries; itry++) {
		if ( displayDD(ddtablehtml, parhtml) ) {
			console.log('BBO Helper: Waited %d mS for BBO Handviewer to finish ' + 
				'rendering after receiving double dummy data.', itry * sleepTime);
			return;
		}
		await sleep(sleepTime);
	}
	
	console.warn('BBO Helper doubleDummyCallback(): ' +
...
function displayDD(ddtablehtml, parhtml) {
	...
	if ( document.getElementById('bhDoubleDummy') !== null ) { return true; }
	
	// Create a <div> containing the double dummy and par information.
	let theDiv = document.getElementById('theDiv');
	if (theDiv === undefined) { return; }   // Shouldn't happen, just a guard
	
	// Bail if BBO page rendering is not done.
	if (theDiv.style.width === '') { return; }
	...
	for (let fontSize = 150; fontSize > 50; fontSize -= 10) {
		...
		if (dv.offsetWidth < 0.96 * maxWidth && dv.offsetHeight < maxHeight ) { break; }
	}
...
async function analyze() {
	...
	if (app.lin !== undefined) {
		...
		console.info('BBO Helper analyze(): Launching BSOL2 in a new tab: %s', BSOLurl);
		window.open(BSOLurl);
	}
	else {
		let d = getDealViaDOM();
		...
		if (d.deal.length !== 67 && fulldeal !== undefined && fulldeal.length === 67) {
			d.deal = fulldeal;
			d.hand = fulldeal.split(':');
		}
		
		if (app.datestr) { d.title = app.datestr + ' BBO Session'; }
...
async function createpbn() {
	...
	if (app.lin === undefined) {
		...
		if ( auctionStr !== null ) {
			...
			let auction = [];
			let bidCnt = 0, lastBid, lastBidIx;
			for (let ix=0; ix<auctionStr.length; ix++) {
				...
				if ( c.charCodeAt(0) < 49 || c.charCodeAt(0) > 55 ) {
					...
					bidCnt++;
					const bid = auctionStr.slice(ix, ix+2);
					auction.push(bid);
					lastBid = bid;
			...
			let fromACBL = p.get('p') !== null &&
				p.get('p').match(/live\.acbl\.org/i) !== null;
			
			if ( bidCnt > 1 || !fromACBL ) { d.auction = auction; }
			else { 
			...
			if (fromACBL) {
				...
				if (eventMatch !== null) { d.eventname = eventMatch[0]; }
			}
		}
		...
		let pbncomment = "% Generated by BBO Helper browser add-on (Matthew Kidd)\n";
		
		if ( p.get('myhand') !== null ) { pbncomment += '% ' + window.location + '\n'; }
	...
	if (app.when_played !== undefined) {
		...
		let hhmm = zeroPadInt(dt.getHours(), 2) + '.' + zeroPadInt(dt.getMinutes(), 2);
...
async function copyboard(mode) {
	...
	let d = getDealViaDOM();
	if (app.datestr) { d.datestr = app.datestr; }
	if (app.datestr) { d.title = app.datestr + ' BBO Session'; } 
...
async function toggleNameDisplay() {
	// Display (or hides) popup showing real names + location (home state/province 
	// or country) of each player at the table.
	...
	const felt = document.getElementById('theDiv');
	...
	if (app.lin === undefined) { return; }
	
	const pnameMatch = app.lin.match( /(?<=pn\|)[^|]+/ );
	if (pnameMatch === null) { return; }
...
function getDealOnlyViaDOM() {
	...
	for (let i=0; i<4; i++) { hand[i] = getHandViaDOM(el[i]); }
...
function getDealViaDOM() {
	...
	for (let i=0; i<4; i++) { d.name[i] = namedivs[i].innerText; }
	...
	for (let ix=dealerix, i=0; ix<calls.length; ix++, i++) {
		...
		if ( call === '' ) { break; } // Empty <td> elements after final passes
		
		// Alerted calls have a yellow background in the auction box.
		d.alerted[i] = calls[ix].style.getPropertyValue('background-color') === 
			'rgb(255, 206, 0)';
		...
		if (call.length === 2 && call !== 'XX') {
			// Regular call
			let symbol = call.slice(-1);
			let suit = (symbol === '♠') ? 'S' : (symbol === '♥') ? 'H' :
				(symbol === '♦') ? 'D' : 'C';
			call = call.charAt(0) + suit; 
		}
	...
	if ( auctionstr.slice(-2,-1) === ';' ) { 
		auctionstr = auctionstr.slice(0, auctionstr.length-2);
	}
	else {
		auctionstr = auctionstr.slice(0, auctionstr.length-1);
	}
...
function getHandViaDOM(elHandDiv) {	
	...
	let modePicturesOfCards = el[0].childElementCount === 13 &&
	  el[1].childElementCount === 13;
	...
	if (modePicturesOfCards) {
		...
		for (let i=3; i>=0; i--) {
			...
			for (let j=0; j<elc.length; j++) {
				if ( elc[j].childElementCount === 0 ) { continue; }
...
async function alertDivWatcher() {
	// Adds a mutation observer to watch for the creation of <div class="alertDivStyle">,
	// the element that contains the text of an alert when a user clicks on an alerted
	// call. When the text inside the <div> changes, unmangle the text to fix the
	// issue of messed up alerts in Chinese, Cyrillic, Turkish, and other languages
	// that use Unicode codepoints outside the Latin-1 code page.

	const mainDiv = document.getElementById('theDiv');
	if (mainDiv === undefined) { return; }
	
	const obsDivCreation = new MutationObserver(alertDivCreation);
	obsDivCreation.observe(mainDiv, { childList: true } );
	
	
	function alertDivCreation(mutations) {
		for (const mr of mutations) {
			for (const el of mr.addedNodes) {
				if (el.className !== 'alertDivStyle') { continue; }
				// It will only be created once.
				obsDivCreation.disconnect();
				
				const obsDivChange = new MutationObserver(alertDivChange);
				// Note: text is a textNode, i.e. a child as opposed to CharacterData
				obsDivChange.observe(el, { childList: true } );
			}
		}	
	}
	
	function alertDivChange(mutations) {
		for (const mr of mutations) {
			for (const el of mr.addedNodes) {
				// Demangle the text of text node.
				if (el.data === undefined) { continue; }
				const str = UTF8unmangle(el.data);
				// if (str !== el.data) { console.info(el.data); }
				if (str !== el.data) { el.data = str; }
			}
		}
	}
}