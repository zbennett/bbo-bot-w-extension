...
const options = {
	"app": ["appShowAuctionClock", "appShowPlayClock", "appDoubleDummyMode",
		...
		"appAutoAlertsFlag", "appAlertSubstitutions", "appChatNameSubs", 
		"appChatAutoSuits", "appClaimAutoSuits", "appShowPeopleTooltip", 
		"appBoardLocalization", "travSuitFourColor", "sessDoubleDummyAlways"],
...
const pulldownMenuOption = {"boardPlayerNameMode": 
	[   ["bbohandle", browser.i18n.getMessage('options_boardPlayerNameMode_handle') ], 
	  	["name", browser.i18n.getMessage('options_boardPlayerNameMode_name') ], 
      	["seat", browser.i18n.getMessage('options_boardPlayerNameMode_compass') ]
	]};
...
function populate(items) {
	...
	pref = items['pref'];
	if (pref === undefined) {
		// This seems to happen on Chromrium but may be limited to test scenarios
		// where a new version is being loaded as an Unpacked Extension using
		// the developer mode.
		document.body.innerHTML = `
		  <p>Preferences not yet available from local storage.</p>
		  <p>Refresh any BBO related browser tab and try again.</p>`;
		return;
	}
