...
if (isChrome) {
	...
	const width = (lang === 'es') ? 23 : (lang === 'it') ? 17.5 : (lang === 'fr') ? 21 :
	  (lang === 'de') ? 23 : (lang === 'zh') ? 13 : 16;
...
function localize() {
	...
	const elMenu = document.getElementsByClassName('i18n');
...
function showAbout() {
	...
	const appURL = 'https://lajollabridge.com/Software/BBO-Helper/';
	...
	const slogan = browser.i18n.getMessage('about_slogan');
	const viewdoc = browser.i18n.getMessage('about_viewdoc');
	...
	<p><strong>${appname}</strong><br>browser<br>extension</p>
...
async function quickSettings() {
	...
	if (pref === undefined) {
		// This seems to happen on Chromium but may be limited to test scenarios
...
<label for="appDoubleDummyTricks"> ${msg['appDoubleDummyTricks']}</label><br>
	...
	if (isChrome) {
		...
		const width = (lang === 'es') ? 36 : (lang === 'it') ? 30 : 
			(lang === 'fr') ? 34 : (lang === 'de') ? 29 : 24.4;
		document.body.style = `width: ${width}em`;
	}
