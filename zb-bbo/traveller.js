 ...
 * 
 *  - Change title from "Bridge Base Online - Myhands" to "Board # (Event)", 
 *    e.g "Board 5 Traveller (ACBL Open-Pairs Thu 1:10 PM)". Also add similar
 *    <h1> element.
...
function improveTitle() {
	...
	let datestr = tds[1].innerText.slice(0,10);
...
function updateTable() {
	// (1) Makes changes to exising results table, e.g. adding the lead and auction columns
	// (2) Creates a separate results table sorted by auction
	// (3) Returns information decoded from the table.
	...
	thScore.innerHTML = thScore.innerHTML.slice(0,2);
	...
	for (let i=firstResult; i<rows.length; i++) {
		...
		let score = el[9].innerText;
		// parseFloat() conversion here is important for subsequent sorting.
		r.score = parseFloat(isMP ? score.slice(0, score.length-1) : score);
		r.rawscore = el[8].innerText;
		r.result = el[6].innerText;
		r.ix = i;
		
		rs.push(r);
		
		// Tally frequency of each raw score, e.g. +420, -200 etc
		const rawscore = el[8].innerText;
		if (rawscore === 'N/A') { continue; }
		
		if ( rawtally[rawscore] === undefined ) {
			rawtally[rawscore] = { "cnt" : 1, score : r.score };
		}
		else {
			rawtally[rawscore].cnt++;
		}
	}
...
function contractTable(contractArray) {
	...
	for (let i=0; i<contractArray.length; i++) {
		...
		let direction = ca.contract.slice(2);
...
function rawscoreTable(rawtally) {
	// Generate table of raw score frequencies. For compactness the columns
	// are replicated horizontally.
	
	const NREP = 4;  // Number of times to repeat set of columns
	
	const sortedRawScores = Object.keys(rawtally).sort( (a, b) => {return b-a;} );
	const nRawScores = sortedRawScores.length;
	
	let table = document.createElement('table');
	table.id = 'rawscores';
	
	const colClass = ['col-rawscore', 'col-cnt', 'col-score'];
	for (let i=0; i<NREP; i++) {
		for (let j=0; j<colClass.length; j++) {
			let col = document.createElement('col');
			col.className = colClass[j];
			table.append(col);
		}
	}
	...
	let tt = document.createElement('tr');
	let tthtml = browser.i18n.getMessage('traveller_rawscore_freq');
	tt.innerHTML = '<th colspan="' + colClass.length * NREP + '">' + 
		tthtml + '</th>';
	tbody.append(tt);
	
	let th = document.createElement('tr');
	th.innerHTML = '<th>Raw</th> <th>Cnt</th> <th>Score</th> '.repeat(NREP);
	tbody.append(th);
	
	const nrows = Math.ceil(nRawScores / NREP);
	
	for (let i=0; i<nrows; i++) {
		let html = '';
		for (let j=i; j<nrows*NREP; j += nrows) {
			if ( j < nRawScores) {
				const rawscore = sortedRawScores[j];
				const cnt = rawtally[rawscore].cnt;
				const scorestr = rawtally[rawscore].score.toFixed(2);
				
				html += '<td>' + rawscore + '</td>' + '<td>' + cnt + '</td>' +
					'<td>' + scorestr + '</td>';		
			}
			else {
				html += '<td></td>'.repeat( colClass.length );
			}
		}
	...
	// Finally add the table to document.
	const contractTable = document.getElementById('contracts');
	contractTable.insertAdjacentElement('afterend', table);	
}
...
function auctionTable(auctionArray) {
	...
	// Finally add the table to document.
	const rawScoresTable = document.getElementById('rawscores');
	rawScoresTable.insertAdjacentElement('afterend', table);
}
		...
		if (!anybranches) {
			...
			if ( totalChildren > 10 ) {
				...
				if ( continuations.endsWith(sepHTML) ) {
					...
					continuations = continuations.slice(0, len);
				}
				let html = 'then ';
				html += continuations;
...
function auctionTree(rs) {
	...
	function addCall(currentNode, calls, explains, ix) {
		...
		if (call.length === 3) { call = call.slice(0,2); }
...
function linparse(lin) {
	...
	for (let i=0; i<lc.length; i+=2) {
		...
		case 'mb':
			...
			if ( val.slice(-1) === '!' ) { val = val.slice(0, val.length-1); }
	...
	if (ncalls >= 4 && calls[ncalls-1] === 'P' && calls[ncalls-2] === 'P' && 
		...
		else auctionhtml = auctionhtml.slice(1, auctionhtml.length-8);
	}
