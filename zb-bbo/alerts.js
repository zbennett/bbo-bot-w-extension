/* 
 * Programmatically populate the Auto Alert cases options and respond to UI changes.
 *
...
// common.js but we don't 'pull in common.js for Auto Alert functionality).
...
document.addEventListener("change", (e) => {
	...
	if (e.target.type === 'text') {
		...
		// Undo conversion of undefined --> "undefined" when pulldown menu item
		// is set in selectmenu().
...
function populate(items) {
	// Firefox prefixes 'Extension (BBO Helper) - ' to the title, perhaps a security
	// measure to help users from getting into to trouble.
	document.title = browser.i18n.getMessage('alert_auto_alerts');
	
	// Document main <h1> element is special cases that combines two messages.
	document.getElementById('mainheader').innerText = browser.i18n.getMessage('appName') + 
		' ' +  browser.i18n.getMessage('alert_auto_alerts');
			
	// The save button is another special case.	
	document.getElementById('saveButton').value = browser.i18n.getMessage('alert_save');
		
	// Do the routine localization next.
		...
		for (let i=0; i<denomHTML.length; i++) {
			...
			if (bid === '1N') {
				...
				let label = '<span style="font-size: 120%">' + '2' + denomHTML[1] + '/' +  
					denomHTML[2] + '/' + denomHTML[3] + '</span> ' + 
						browser.i18n.getMessage('alert_fourth_seat');
	...
	const forcingNTmenu = [ ["forcing", "Forcing"], ["non-forcing", "Not Forcing"],
	...
	const weak2NTmenu = [ ["feature", "Feature Asking"], ["OGUST", "OGUST"],
		["natural", "Natural"], [undefined, 'Other']];
	...
	for (let i=0; i<twoSuitBids.length; i++) {
		...
		let menu; 
		if (id === 'directCueBid') { 
			menu = [ ["Michaels", "Michaels"], ["Top and Another", "Top and Another"],
	...
	function divExplainCall(call, callHTML, type, id) {
		...
		if (id === 'opening-1NTvul') {
			...
			sp.innerHTML = '(' + browser.i18n.getMessage('alert_diff_vul') + ')';
			dv.append(sp);
		}
	...
	function selectmenu(id, menuitems, initialValue) { 
		...
		for (let j=0; j<menuitems.length; j++) {
			...
			op.value = menuitems[j][0];       // Note undefined --> "undefined"
