...
let aa = {
	"opening" : {"1D": "4+ diamonds unless 4=4=3=2", "3N": "Gambling. Nothing on side",
		"FourthSeat2Bid" : "Full opener with 6+ cards"},
	"nt" : {"JacobyTransfers": true, "TexasTransfers": true, "2S": "Relay to 3!c"},
	"ntdef" : {"d" : "Single-suited", "2C": "!c + higher suit", "2D": "!d + higher suit",
		"2H": "!h + !s", "2S": "Weak spades and/or only 5", "2N": "!c + !d"},
...
browser.storage.local.get('pref').then( onPrefLoad,
	(err) => {
		console.error("BBO Helper: couldn't load user preferences: ", err, ' (using defaults)' );
	}
);
...
function onPrefLoad(item) {
	if ( Object.keys(item).length === 0 ) {
		pref = default_pref;
		browser.storage.local.set( {"pref" : pref} ).then(false, 
			(err) => { console.error('BBO Helper: failed to save preferences: ', err); } );
	}
	else {
		pref = item.pref;
		...
		for (let key in default_pref) {
			if (pref[key] === undefined) { pref[key] = default_pref[key]; newkeys++; }
		}
		if (newkeys) {
			browser.storage.local.set( {"pref" : pref} ).then(false, 
				(err) => { console.error('BBO Helper: failed to save preferences: ', err); } );			
	...
	if (isChrome) {
		...
		document.dispatchEvent( new CustomEvent("pref_update", {"detail": clonedPref}) );
	}	
...
function onPrefUpdate(item) {
	...
	if (isChrome) {
		document.dispatchEvent( new CustomEvent("pref_update", {"detail": item.pref}) );
	}
	else {
...
document.addEventListener('pref_request', () => {
	console.info('BBO Helper: responding to a request for user preferences');
...
function realnameResponse(msg) {
	if (msg === undefined) {
		...
		console.error('BBO Helper: realnameResponse() received undefined from service worker.');
	}
		...
		for (let i=0; i<msg.bbohandle.length; i++) {
			if ( msg.fail[i] ) { realnames[msg.bbohandle[i]] = undefined; continue; }
			
			let p = { "fullname": msg.fullname[i], "state": msg.state[i], 
				"mp": msg.mp[i], "pnum": msg.pnum[i] };
					
			realnames[msg.bbohandle[i]] = p;		
		}
	}
	
	// Single BBO handle response.
	else if (msg.lookupfail) {
		// Record that lookup failed so that we don't try again.
		realnames[msg.bbohandle] = undefined;
	}
	else {
		const bbohandle = msg.bbohandle;
		delete msg['bbohandle'];
		realnames[bbohandle] = msg;
	}
}
...
// TP suffix indicate the "text presentation" versions that explicitly suppress the Emoji 
...
const suitentityTP = suitentity.map( (a) => { return a + '\uFE0E'; } );
...
const suitHTMLplain = new Array(4), suitHTML4color = new Array(4);
const suitHTMLplainTP = new Array(4), suitHTML4colorTP = new Array(4);

for (let i=0; i<4; i++) {
	suitHTML4color[i] = `<span style="color: ${suit4color[i]}">${suitentity[i]}</span>`;
	suitHTML4colorTP[i] = `<span style="color: ${suit4color[i]}">` +
		suitentityTP[i] + '</span>';
	...
	if ( suit2color[i] === '' ) {
		...
		suitHTMLplain[i] = `<span style="color: ${suit2color[i]}">${suitentity[i]}</span>`;
		suitHTMLplainTP[i] = `<span style="color: ${suit2color[i]}">` + 
			suitentityTP[i] + '</span>';
	}
}
...
function bestAvailableName(bbohandle) {
	...
	if ( realnames[lchandle] ) { return realnames[lchandle].fullname; }
	
	if ( !bboprofiles[lchandle] ) { return bbohandle; }
	
	let bboname = bboprofiles[lchandle].name;
	
	// NAME field should always be present but guard against BBO changes.
	if ( bboname === undefined ) { return bbohandle; }
	...
	// User has some semblance of a real name in their BBO profile. 
	// First remove any spurious white space.
	let parts = bboname.split(/\s+/);
	
	// Some players add a suffix like GLM (Gold Life Master) to their name.
	// Remove this. B = Bronze, S = Silver, R = Ruby, G = Gold, D = Diamond,
	// E = Emerald. Others add a player number for their national bridge 
	// organization (NBO). Strip these.
	if (parts.length > 1 && parts[parts.length-1].match(/^([BSRGDE]?LM|\d+)$/) ) { 
		parts.pop();
	}
	
	// Attempt to address capitalization issues.
	for (let i=0; i<parts.length; i++) {
		let part = parts[i];
		// Fix obnoxiously fully capitalized names put ignore suffixes like III
		if ( part.length > 1 && part.match(/^[A-Z]+$/) !== null &&
			part.match(/^[IV]+$/) === null )  { part = part.toLowerCase(); }
		
		// Capitalize first character of Latin alphabet names except for nobiliary particles.
		if ( (part.charAt(0) >= 'a' && part.charAt(0) <= 'z') && part !== 'de' &&
			part !== 'von' && part !== 'zu' && part !== 'of' ) {
	...
	return parts.length === 1 ? parts[0] + ' (' + bbohandle + ')' : parts.join(' ');
}
...
async function boardhtml(d, inlineCSS, includeAll=false) {
	...
	// Holds CSS class or style depending on whether styling is being inlined.
	let cos;
	
	const seatlabels = seatLabels(d.name, d.isVugraph);

	const localize = pref.appBoardLocalization && app.lang !== 'en' && d.handL !== undefined;
	let [west, north, east, south] = localize ? 
		[d.handL[1], d.handL[2], d.handL[3], d.handL[0]] : d.deal.split(':');
	
	const suithtml = !inlineCSS ? suitHTMLclass : 
		pref.suitFourColor ? suitHTML4colorTP : suitHTMLplainTP;
	
	const showTiming = (pref.boardShowTiming || includeAll) && 
		d.auctionTimes !== undefined && d.playTimes !== undefined;
	
	const showDD = (pref.boardShowDoubleDummy || includeAll) && (d.dd !== undefined);
	...
	const showPlay = (pref.boardShowPlay || includeAll) && isFullDeal &&
		(d.cardplay !== undefined || d.lin !== undefined && d.lin.indexOf('pc|') !== -1);

	if (showPlay) { playHTML = cardhtml(d, showTiming, inlineCSS); }
	
	let showExplanations = (pref.boardShowExplanations || includeAll) && isFullDeal && d.alert;
	if (showExplanations) {
		let alert = new Array(d.alert.length);
		if (!pref.boardHideRobotExplanations) { alert = d.alert; }
		else {
			// Clear Robot alerts.
	...
	if (d.auctionstr !== undefined) { dataAttribs += ` data-auction="${d.auctionstr}"`; }
	
	let data_contract = d.contractLevel === -1 ? 'Incomplete Auction' : 
		d.contractLevel === 0 ? 'Passed Out' : 
	...
	if (inlineCSS) {
		...
		if (pref.boardIncludeBorder) { dvstyle += '; border: 1px solid #777'; }
		cos = `style="${dvstyle}" `;
	}
	...
	// North hand
	html += `<td class="bh-north" style="padding-left: ${nspadding}">` + 
		handhtml(north, seatlabels[2], inlineCSS) + '</td>' + "\n";
	
	// Strip off the day (e.g. 'Sun') at the start.
	let datehtml = d.datestr === undefined || !pref.boardShowDateTime ? '' : 
		'<span style="font-size: 80%">' + d.datestr.slice(4) + '</span><br>';
	...
	if (!isFullDeal || (!pref.boardShowLinks && !includeAll) ) {
		...
		if (d.score !== undefined) {
			...
			const cos = inlineCSS ? 'style="margin-bottom: 0.3em"' : 'class="bh-score"';
		...
		// BSOL doesn't seem to like [] in '[B++]', even when properly encoded. So replace
		// with parentheses.
		const encodedLIN2 = encodeURIComponent(lin.replaceAll('[B++]', '(B++)'));
		
		let HVurl = 'https://www.bridgebase.com/tools/handviewer.html?lin=' + encodedLIN;
		let BSOLurl = 'https://dds.bridgewebs.com/bsol2/ddummy.htm' + '?lin=' + encodedLIN2;
		...
		let style = 'padding-left: 0.7em; vertical-align: ' + (alignTop ? 'top' : 'middle');
	...
	if (showPlay) { 
		html += '<td rowspan="3" style="padding-left: 1.5em">' + '\n' + playHTML + '</td>';
	}
	...
	if (showExplanations) {
		...
		html += `<td rowspan="3" class="bh-explanations" style="${style}">` + explainHTML + '</td>';
	}
		
	html += '</tr>' + "\n\n";
	
	style = `vertical-align: middle; padding-right: ${ewpadding}`;
	html += '<tr>' + `<td class="bh-west" style="${style}">` +
		handhtml(west, seatlabels[1], inlineCSS) + '</td>' + "\n";
	...
	html += `<td class="bh-east" style="${style}">` + 
		handhtml(east, seatlabels[3], inlineCSS) + '</td></tr>' + "\n\n";
	...
	if (showDD) {
		...
		html += '<td><div class="bh-dd-par">' + ddtablehtml + "\n" + parhtml + '</div></td>';
	}
	...
	html += `<td class="bh-south" style="padding-left: ${nspadding}">` + 
		handhtml(south, seatlabels[0], inlineCSS) + '</td>' + "\n";
	
	const showContract = isFullDeal ? (pref.boardShowContract || includeAll) : 
	...
	if (!showContract) {
		...
		let cos = inlineCSS ? 'style="text-align: center; vertical-align: middle"' : 
		...
		if (d.contractLevel === -1) {
			...
			const denomtxt = ix !== -1 ? suithtml[ix] : localize ? app.locale.nt : 'NT';
			const declarertxt = !localize ? d.declarer :
		...
		if (showHCP) { html += hcphtml(d.hcp, inlineCSS); }	
...
function numvulhtml(bstr, vul) {
	...
	const bstyle = `text-align: center; vertical-align: middle; ` +
	...
	let tstyle = 'display: flex; justify-content: center; align-items: center; ' +
		`font-size: ${fontsize}%`;
	...
	if (vul === 'NS') {
		...
		let style = `background-color: red; width: ${w}em; height: ${h}em; ` + 
			'display: flex; justify-content: center; align-items: center';

		return `<td style="${tstyle}">` + 
			`<div style="${style}">` + bnhtml + '</div></td>';
	}
	...
	console.log("BBO Helper: numvulhtml(): vul must be 'None', 'NS', 'EW' or 'All'");
	return '';
}
...
function handhtml(hd, pname, inlineCSS) {
	...
	if (pref.boardIncludeNames) {
		cos = inlineCSS ? 'style="background-color: #d3d3d3; padding: 0 0.2em 0 0.2em"' :
			'class="pname"';
		html += `<span ${cos}>` + pname + '</span><br>'; 
	}
	
	html += inlineCSS ? '<span style="letter-spacing: 0.2em">' : '<span class="hand">';
	...
	for (let i=0; i<4; i++) {
		...
		if (inlineCSS) {
			...
			if ( suitcolor[i] !== '' ) { style += `; color: ${suitcolor[i]}`; }
			cos = `style="${style}"`;
		}
		else {
		...
		if (suits[i].length) {
			if (!pref.cardUse10) { html += suits[i]; }
			else {
			...
			// Void in suit.
			if (pref.handVoidmdash) { html += '&mdash;'; }
		}
		if (i<3) { html += '<br>'; }
	}
	html += '</span>';
...
function timecolor(tm, inlineCSS) {
	...
	if (tm <= pref.timingInstantAction) { return inlineCSS ? '#aaffff' : 'insta'; }
	else if (tm >= pref.timimgLongBIT) { return inlineCSS ? '#ff7f7f' : 'longBIT'; }
	else if (tm >= pref.timingBIT) { return inlineCSS ? '#ffd47f' : 'BIT'; }
}
...
function auctionhtml(d, showTiming, inlineCSS) {
	...
	// Non-contract calls
	const txtPass = pref.auctionShortCalls ? 'P' : localize ? app.locale.pass : 'Pass';
	const txtDouble = pref.auctionShortCalls ? 'X' : localize ? app.locale.dbl : 'Dbl';
	const txtRedouble = pref.auctionShortCalls ? 'XX' : localize ? app.locale.rdbl : 'Rdbl';
	...
	const seatnames = localize ?
		( pref.auctionSeatLetters ?
		  [ app.locale.seatLetters.charAt(1), app.locale.seatLetters.charAt(2),
			app.locale.seatLetters.charAt(3), app.locale.seatLetters.charAt(0) ] :
		  [ app.locale.seatName[1], app.locale.seatName[2], 
		  	app.locale.seatName[3], app.locale.seatName[0] ]
		) :
		( pref.auctionSeatLetters ? ['W', 'N', 'E', 'S'] : ['West', 'North', 'East', 'South'] );
	
	let style = 'text-align: center; border-collapse: collapse';
	if (bboStyling) {
		style += '; border: 1px solid #aaa; background-color: ' + bboAuctionBoxColor;
	}
	
	let classList = 'bh-auction';
	if (showTiming) { classList += ' ' + 'bh-auctiontm'; }	
	
	let html = `<table class="${classList}" style="${style}">` + "\n";
	// Auction table begins with West (BBO convention).
	let colwidth = !pref.auctionSeatLetters ? 2.6 : !pref.auctionShortCalls ? 2 : 1.6;
	...
	if (!bboStyling) {
		...
		let styleV  = wstyle + '; ' + 'background-color: ' + bboVulColor + '; ' + 'color: white';
		let styleEW = d.vul === 'All' || d.vul === 'EW' ? styleV : styleNV;
		let styleNS = d.vul === 'All' || d.vul === 'NS' ? styleV : styleNV;
		html += 
			`<th style="${styleEW}"${colspan}>` + seatnames[0] + '</th>' +
			`<th style="${styleNS}"${colspan}>` + seatnames[1] + '</th>' +
			`<th style="${styleEW}"${colspan}>` + seatnames[2] + '</th>' +
			`<th style="${styleNS}"${colspan}>` + seatnames[3] + '</th>';
	}
	html += '</tr></thead>';
	html += '<tbody>' + "\n";
	
	// Figure out number of empty <td> elements before dealer.
	let nEmpty = d.dealer === 'W' ? 0 : d.dealer === 'N' ? 1 : d.dealer === 'E' ? 2 : 3;
	...
	html += '<tr>' + (showTiming ? '<td></td><td class="tma"></td>' : '<td></td>').repeat(nEmpty);
	...
	if (!showTiming && pref.auctionHideFinalPasses && d.auction.length > 3) {
		if ( d.auction[lix-2] === 'P' && d.auction[lix-1] === 'P' && 
				d.auction[lix] === 'P' && !(lix == 3 && d.auction[0] === 'P') ) {
			lix -= 3;
		}
	}
	...
	for (let i=0; i<=lix; i++) {
		...
		if (call === 'P') { callhtml = txtPass; }
		else if (call === 'X') { callhtml = txtDouble; }
		else if (call === 'XX') { callhtml = txtRedouble; }
		else {
		...
		if (showTiming) {
			...
			if (ms === 0 || ms === undefined) { 
				html += inlineCSS ? '<td style="border-left: none"></td>' : '<td class="tma"></td>';
				continue;
			}
			
			// Longer than 552.955 seconds (nearly 11 minutes!)
			else if (ms === 655350) { tmstr = '&infin;'; color = inlineCSS ? 'red' : 'infinity'; }
			...
			// Basic styling for timing information.
			const style = 'border-left: none; margin-left: 0; text-align: left; ' + 
				'font-size: 65%; vertical-align: top';
			cos = inlineCSS ? `style="${style}"` : 'class="tma"';	
			
			if (color === undefined) { tmHTML = tmstr; }
			else {
				let tmcos = inlineCSS ? `style="background-color: ${color}"` : `class="${color}"`;
	...
	if (nEmptyCalls !== 0) {
		let rephtml = '<td></td>';
		if (showTiming) {
			rephtml += inlineCSS ? '<td style="border-left: none"></td>' : '<td class="tma"></td>';
		}
...
function cardhtml(d, showTiming, inlineCSS) {
	...
	let nclaimed = d.nclaimed !== undefined ? d.nclaimed :
		 d.lin === undefined ? undefined : lin2claimed(d.lin);
	...
	for (let i=0; i<cardplay.length; i+=4) {
		...
		if (i+1 < cardplay.length ) { cd[d.whohas[cardplay[i+1]]] = cardplay[i+1]; }
		if (i+2 < cardplay.length ) { cd[d.whohas[cardplay[i+2]]] = cardplay[i+2]; }
		if (i+3 < cardplay.length ) { cd[d.whohas[cardplay[i+3]]] = cardplay[i+3]; }
		...
		for (let j=0; j<4; j++) {
			...
			if (cd[jr] === undefined) {
				...
				html += '<td></td>';
				if (showTiming) { html += inlineCSS ? 
					'<td style="border-left: none"></td>' : '<td class="tm"></td>'; }
				continue;				
			}	
			let suit = cd[jr].charAt(0);
			
			ix = (suit === 'S') ? 0 : (suit === 'H') ? 1 : (suit === 'D') ? 2 : 3;
			cos = showTiming ? (inlineCSS ? 'border-right: none; margin-right: 0' : 'cd') : '';
			
			if (jr === leadseat) {
				if (cos !== '') { cos += inlineCSS ? '; ' : ' '; }
				if (inlineCSS) { cos += 'background-color: #ccffcc'; }
				else { cos += 'leader'; }
			}
				
			if (cos !== '') { cos = ' ' + (inlineCSS ? 'style' : 'class') + `="${cos}"`; }
			
			let cardrank = cd[jr].charAt(1);
			if (localize) {
				let ih = 'JQKA'.indexOf(cardrank);
				if (ih !== -1) { cardrank = app.locale.honorLetters.charAt(ih); }
			}
			...
			if (showTiming) {
				...
				if (ms !== undefined) { seatTimeTotals[j] += ms; }
				...
				if (ms === 0 || ms === undefined) {
					html += inlineCSS ? '<td style="border-left: none"></td>' : '<td class="tm"></td>';
					continue;
				}
	...
	if (showTiming) {
		...
		html += `<tr ${cos}>`;
		for (let j=0; j<4; j++) {
			html += '<td colspan="2">' + (seatTimeTotals[j]/1000).toFixed(1) + '</td>';
		}
		html += '</tr>';
	}
	...
	if (nclaimed !== undefined) {
		...
		html += `<tr ${cos}><td colspan="${ncols}">` + 
			nclaimed + ' Tricks Claimed' + '</td></tr>';
	}
...
function ddhtml(dd, mode, inlineCSS) {
	...
	const localize = (pref.appBoardLocalization || mode === 'History') && 
		app.lang !== 'en';
	
	const seatorder = 'NSEW';  // BSOL double dummy table order
	const seatorderL = !localize ? seatorder : 
		app.locale.seatLetters[2] + app.locale.seatLetters[0] + 
		app.locale.seatLetters[3] + app.locale.seatLetters[1];
	...
	if (mode === 'Board') { style += '; font-size: 60%'; }
	let html = `<table class="bh-dd" style="${style}">` + "\n" +
	...
	for (let i=3; i>=0; i--) {
		...
		if (inlineCSS) {
			...
			html += '<span style="font-size: 120%">' + 
				(mode === 'Handviewer' || mode === 'History' ? 
				suitHTMLplainTP[i] : suithtml[i]) + '</span>';
		}
		else {
	...
	html += '<td style="width: 1.8em">' + textNT + '</td>' + '</tr></thead>' + "\n";
	html += '<tbody>';
	
	// Loop over seats
	let tdstr;
	let ddcos = inlineCSS ? `style="border: ${borderWidth} solid #444"` : 'class="dd"';
	for (let j=0; j<4; j++) {
		html += '<tr>';
		html += inlineCSS ? '<td style="border: none"><b>' + seatorderL.charAt(j) + '</b></td>' :
		...
		for (let i=4; i>=0; i--) {
			...
			if (pref.appDoubleDummyTricks) { tdstr = ntricks; }
			else { tdstr = ntricks > 6 ? ntricks - 6 : '-'; }
	...
	if ( dd.sNS == 0 ) {
		...
		const NStext = seatorderL.slice(0,2);
		const EWtext = seatorderL.slice(2,4);
		parScoreText = NStext + ': ' + dd.sNS + ' ' + EWtext + ': ' + dd.sEW;
		parContractText = NStext + ': ' + parcontracts(dd.cNS, dd.tr) + 
			' ' + EWtext + ': ' + parcontracts(dd.cEW, dd.tr);
	}
	
	// For the copy-and-paste aid and session HTML, do not include the 'Par: ' in front 
	// of the contract list to save real estate.
	let parhtml = 'Par Score: ' + parScoreText + '<br>' + 
		(mode === 'Board' ? '' : 'Par: ') + parContractText; 
	
	parhtml = inlineCSS ? '<strong>' + parhtml + '</strong>' : 
	...
	function parcontracts(str, tr) {
		...
		for (let i=0; i<contracts.length; i++) {
			...
			// 'NS' and 'EW' first.
			const seatsLocalized = !localize ? seats :
				seats === 'NS' ? seatorderL.slice(0,2) :
				seats === 'EW' ? seatorderL.slice(2,4) :
			...
			let ix = 0;
			while (c.charCodeAt(ix+1) >= 49 && c.charCodeAt(ix+1) <= 55) { ix++; }
...
function explainhtml(auction, alerts, showPlay, inlineCSS) {
	let html = '';
	...
	// This text at the end of an alert means BBO Helper automatically generated
	// the alert.
	const alertSuffix = ' [B++]';

	// Figure out how much alert text there is and if there are an autoAlerts.
	let nchar = 0;
	let anyAutoAlerts = false;
	for (let i=0; i<alerts.length; i++) {
		if (alerts[i] === undefined || alerts[i].trim() === '') { continue; }	
		nchar += alerts[i].length;
		if ( alerts[i].endsWith(alertSuffix) ) { anyAutoAlerts = true; }
	}
	
	const alertWidth = !showPlay || nchar >= 180 ? 12 : anyAutoAlerts ? 8 : 7;
	const style = `margin-bottom: 0.3em; max-width: ${alertWidth}em`; 
	
	html += `<div style="${style}; text-decoration: underline">` + 
		browser.i18n.getMessage('explanations') + '</div>' + '\n';

	const bhaStyle = 'background-color: #061e3d; color: #f7c800; vertical-align: 10%; ' + 
		'font-size: 70%; font-weight: bold; padding-left: 0.1em; padding-right: 0.2em;';
	const bhaplusStyle = 'font-size: 75%; position: relative; ' + 
		'vertical-align: baseline; top: -0.30em; letter-spacing: -0.2em;';

	for (let i=0; i<alerts.length; i++) {
		let alert = alerts[i];
		if (alert === undefined) { continue; }
		alert = alert.trim();
		const autoAlerted = alert.endsWith(alertSuffix);
		
		if ( autoAlerted ) {
			alert = alert.slice(0, alert.length - alertSuffix.length );
		}
		...
		alertHTML = alert.replace( /![cdhs]/gi, symsub );
		...
		if ( autoAlerted ) {
			// Add stylized B++ icon without overhead of using an actual image.
			if (inlineCSS) {
				alertHTML += ` <span style="${bhaStyle}">B` +
					`<span style="${bhaplusStyle}">++</span></span>`;
			}
			else {
				alertHTML += ' <span class="bha">B<span class="bhaplus">++</span></span>';
			}
		}
		...
		if (call === 'P') { callhtml = txtPass; }
			...
			let hsym = denom === 'N' ? pref.auctionTextNT : 
				denom === 'S' ? suithtml[0] : denom === 'H' ? suithtml[1] : 
				denom === 'D' ? suithtml[2] : suithtml[3];
			callhtml = call.charAt(0) + hsym;
		}
		
		html += `<div style="${style}">` + callhtml + ' = ' + 
			alertHTML + '</div>' + '\n';
	}
...
function dealHCP(d) {
	...
	for (let i=0; i<d.hand.length; i++) {
		let hand = d.hand[i], hcp = 0, suit = 0;
...
function contract(d) {
	...
	if (nbids === 4 && au[0] === 'P' && au[1] === 'P' && au[2] === 'P' && au[3] === 'P') {
		// Passed out
		d.contractLevel = 0; d.contractDenom = ''; d.doubled = ''; d.declarer = '';
	}
	else if (au.length < 4 || au[nbids-3] !== 'P' ) {
		// Incomplete auction
		d.contractLevel = -1; d.contractDenom = ''; d.doubled = ''; d.declarer = '';
	}
	else {
		let dix;
		let doubled = '';
		for (let i=nbids-4; i>=0; i--) {
			...
			else if (call === 'X' && doubled === '') { doubled = 'x'; }
		}
		d.doubled = doubled;
		...
		for (let i=dix; i<nbids-3; i+=2) {
			...
			if (call.charCodeAt(0) >= 49 && call.charCodeAt(0) <= 55 && 
				d.contractDenom === au[i].charAt(1)) {
				d.declarer = seatletters.charAt( (seatletters.indexOf(d.dealer) + i) % 4 );
				break;
			}
		}
...
function seatLabels(name, isVugraph) {
	...
	if ( pref.boardPlayerNameMode === 'seat' ) {
		const localize = pref.appBoardLocalization && app.lang !== 'en';
		seatlabels = localize ? app.locale.seatName : ['South', 'West', 'North', 'East'];
	}
		...
		if ( isVugraph ) {
			...
			// Typical case
			for (let i=0; i<4; i++) { seatlabels[i] = bestAvailableName( name[i] ); }
		}
	}
	else {
		console.error('BBO Helper: seatlabels() unsupported boardPlayerNameMode: ',
			pref.boardPlayerNameMode, 'Defaulting to BBO handles');
...
function deal2pbn(d) {
	...
	if (!isNaN(d.bnum)) { pbn += '[Board "' + d.bnum + '"]' + "\n"; }
	...
	pbn += '[Deal "' + 'W:' + d.hand[1] + ' ' + d.hand[2] + ' ' + d.hand[3] + ' ' +
	  d.hand[0] +  '"]' + "\n";

	// Adds contractLevel, contractDenom, doubled, and declarer fields
	if (!d.contract && d.auction !== undefined) { d = contract(d); } 

	if (d.declarer) { pbn += '[Declarer "' + d.declarer + '"]' + "\n"; }

	if (d.contract) {
		let pbncontract = d.contract;
		// PBN format requires NT for notrump contracts.
		if (pbncontract.charAt(1) === 'N' && pbncontract.length === 2) { pbncontract += 'T'; }
		...
		for (let i=0; i<cardplay.length; i+=4) {
			...
			for (let j=0; j<4; j++) {
				if (i+j < cardplay.length) {
					...
					if (d.playTimes) { tm[seatix] = d.playTimes[i+j]; }
				}
			}
			if (d.playTimes) { playTimes = playTimes.concat(tm); }
	...
	if (d.dd !== undefined) {
		...
		for (let i=0; i<4; i++) {
			for (let j=0; j<5; j++) {
				...
				pbn += ddSeats[i] + ddDenom[j] + (ntricks < 10 ? '  ' : ' ') + ntricks + '\n';
			}
		}		
	}
	...
	if (d.auctionTimes && d.auctionTimes.length > 0) {
		...
		if (d.auctionTimes.length % 4 !== 0) { pbn += '\n'; }
	}	
...
function deal2lin(d, seatlabels) {
	...
	if ( pref.boardPlayerNameMode === 'bbohandle' ) {
		...
		// Remove commas and pipe symbols from names (yes both happen in practice,
		// e.g. "Mike | 2/1 or SAYC"), to avoid screwing up the LIN output.
		let cleanlabels = new Array(4);
		for (let i=0; i<4; i++) { cleanlabels[i] = seatlabels[i].replace(/[,|]/g, ''); }
		lin += 'pn|' + cleanlabels.join() + '|';
	}
	
	// First digit after 'md|' indicates dealer (1 = South, 2 = West, ...)
	let lindix = seatletters.indexOf(d.dealer) + 1;
	lin += 'md|' + lindix + dothand2linhand(d.hand[0]) + ',' + dothand2linhand(d.hand[1]) + ',' +
		dothand2linhand(d.hand[2]) + ',' + dothand2linhand(d.hand[3]) + '|';
	let linvul = d.vul === 'All' ? 'b' : d.vul === 'NS' ? 'n' : d.vul === 'EW' ? 'e' : 'o';
	lin += 'sv|' + linvul + '|rh||ah|Board ' + d.bnum + '|';
	
	for (let i=0; i<d.auction.length; i++) { 
		let bid = d.auction[i];
		if (bid === 'X') {bid = 'd'; } else if (bid === 'XX') { bid = 'r'; }
		lin += 'mb|' + bid + '|';
		if (d.alert !== undefined && d.alert[i]) { lin += 'an|' + d.alert[i] + '|'; }
	}
	if (d.cardplay) {
...
async function lin2pbn(lin, epochTime, eventname, dtricks, isMP, rawScoreNS, scoreNS) {
	...
	let d = {};
	const namelist = lin.match( /pn\|[^|]*/ );
	d.name = namelist === null ? ['', '', '', ''] : namelist[0].slice(3).split(',');
	...
	const ddkey = 'dd' + d.hand[0] + ':' + d.hand[1] + ':' + d.hand[2];
	const item = await browser.storage.local.get(ddkey);
	const dd = item[ddkey];
	
	// Get timing information (if available). Keyed by hand-bbohandle for
	// one of seats (don't know which).
	const timing = await getDealTiming(d.hand, d.name);
	...
	// Case insensitive. BBO itself is not consistent. For example Daylong movie links
	// use uppercase vulnerability in the movie URL.
	let linVul = lin.match( /(?<=sv\|)[oneb]/i );
	d.vul = linVul !== null ? LINvul2PBNLvul[ linVul[0].toLowerCase() ] : 'None';
	...
	if (epochTime !== undefined) {
		...
		let yyyymmdd = dt.getFullYear() + '.' + zeroPadInt(dt.getMonth() + 1, 2) +
			'.' + zeroPadInt(dt.getDate(), 2);
		let hhmmss = zeroPadInt(dt.getHours(), 2) + ':' + zeroPadInt(dt.getMinutes(), 2) +
		...
		let UTCyyyymmdd = dt.getUTCFullYear() + '.' + zeroPadInt(dt.getUTCMonth() + 1, 2) +
			'.' + zeroPadInt(dt.getUTCDate(), 2);
		let UTChhmmss = zeroPadInt(dt.getUTCHours(), 2) + ':' +
			zeroPadInt(dt.getUTCMinutes(), 2) + ':' + zeroPadInt(dt.getSeconds(), 2);
	...
	if (scoreNS !== undefined) {
		pbn += (isMP ? '[ScorePercentage ' : '[ScoreIMP ') + '"NS ' + scoreNS + '"]' + "\n";
	}
	...
	if (d.auction.length !== 0) {
		...
		for (let i=0; i<d.auction.length; i++) {
			...
			let pbncall = call.length === 2 ? call : call === 'P' ? 'Pass' : 
				call === 'D' ? 'X' : call === 'R' ? 'XX' : '-';
			// PBN format requires NT for notrump bids.
			if ( pbncall.charAt(1) === 'N' ) { pbncall += 'T'; }
		...
		for (let i=0; i<cardplay.length; i+=4) {
			...
			for (let j=0; j<4; j++) {
				if (i+j < cardplay.length) {
					let seatix = d.whohas[cardplay[i+j]];
					cd[seatix] = cardplay[i+j];
					if (timing) { tm[seatix] = timing.playTimes[i+j]; }
				}
			}
			if (timing) { playTimes = playTimes.concat(tm); }
	...
	if (timing && timing.auctionTimes.length > 0) {
		...
		if (timing.auctionTimes.length % 4 !== 0) { pbn += '\n'; }
	}
	...
	if (playTimes.length > 0) {
		...
		for (let i=0; i<playTimes.length; i++) {
			let tm = playTimes[i];
			if (tm === undefined) { pbn += ' -';  }  // incomplete last trick.
			else {			
...
function lin2auction(lin) {
	...
	let lc = lin.slice(0,lin.length-1).split('|');
	...
	let completedAuction = auction.length >= 4 && auction[nc-1].toUpperCase() === 'P' &&
		auction[nc-2].toUpperCase() === 'P' && auction[nc-3].toUpperCase() === 'P';
	
	if (auction.length === 4 && completedAuction && auction[0].toUpperCase() === 'P') {
		...
		for (i1=auction.length-4; i1>= 0; i1--) {
			...
			if (auction[i1].length === 2) { break; }
		}
		
		// PBN format requires NT for notrump contracts.
		pbncontract = auction[i1].toUpperCase();
		if (pbncontract.charAt(1) === 'N') { pbncontract += 'T'; }
		...
		for (let i2=auction.length-4; i2>i1; i2--) {
			...
			if (call === 'd') { pbncontract += 'X'; break; }
			if (call === 'r') { pbncontract += 'XX'; break; }
		}
		...
		for (dix = (i1 % 2); dix<auction.length-3; dix+=2) {
			if (auction[dix].charAt(1) === denom) { break; }
		}
		dix = dix % 4;
	}
...
function lin2cardplay(lin) {
	...
	let lc = lin.slice(0,lin.length-1).toUpperCase().split('|');

	for (let i=0; i<lc.length; i+=2) {
		if ( lc[i] === 'PC' ) { cardplay.push(lc[i+1]); }
	}
	return cardplay;
}
...
function lin2hands(lin) {
	// LIN format may not include the East hand because it must contain whatever
	// cards are left. This function parses LIN for the hands, deriving the East
	// hand if necessary.
	const rankorder = '23456789TJQKA';
	
	const ix1 = lin.search( /\|md\|/i );
	if (ix1 === -1) { return; }
	
	// Have to be careful about how we test for 3-handed and 4-handed version because
	// the 3-handed version comes in two varieties (with and without trailing comma)
	// depending on where it is generated by BBO, i.e. these two cases.
	//
	// |md|4S49QKAHTD25679C59,S367JH34569DCJQKA,S8TH28JAD4TAC2347|
	// |md|4S49QKAHTD25679C59,S367JH34569DCJQKA,S8TH28JAD4TAC2347,|
	
	let ix2 = lin.indexOf('|', ix1+4);
	// Guard against missing trailing '|' in LIN string with no auction, e.g. time
	// ran out. Not known to be an issue, but BBO is not always consistent. 
	if (ix2 === -1) { ix2 = lin.length; }

	if ( (ix2 - ix1) === 76 ) {  // 71 (see below) + '|md|#'.length()
		// Four handed version
		// 71 = (13 cards + 4 suit letters) * 4 hands + 3 commas
		return lin.slice(ix1+5, ix1+76).split(',');
	}
	
	// Three handed version.
	// 53 = (13 cards + 4 suit letters) * 3 hands + 2 commas
	let hand = lin.slice(ix1+5, ix1+58).split(',');
	...
	for (let i=0; i<52; i++) {
		if ( cards[i] ) { continue; }
		while ( i >= suitix * 13 ) { east += suitrank.charAt(suitix); suitix++; }
...
function linhand2dothand(hand) {
	...
	for (let i=0; i<4; i++) {
		...
		suit = unorderedSuits[i].slice(1);
		
		if ( suit.length < 2 ) { suits[ir] = suit; continue; }   // No sorting needed
		let ixlist = [];
		for (let j=0; j<suit.length; j++) {
			ixlist.push( rankorder.indexOf(suit.charAt(j)) );
		}
		ixlist.sort( function(a,b) {return a-b} );

		for (let j=0; j<ixlist.length; j++) { suits[ir] += rankorder.charAt(ixlist[j]); }
	}
	return suits.join('.');
}
...
function linboard2dotboard(hd) {
	...
	let hd2 = new Array(4);
	for (let i=0; i<4; i++) { hd2[i] = linhand2dothand( hd[i] ) }
	return hd2;
}
...
function rotatedeal(d, nseats) {
	...
	if (d.handL !== undefined) {
		let handL = [...d.handL];
		for (let i=0; i<4; i++) { ix = (i + nseats) % 4; d.handL[ix] = handL[i]; }
	}
	...
	d.declarer = seatletters.charAt( (seatletters.indexOf(d.declarer) + nseats) % 4 );
	...
	if ( nseats % 2 === 1 ) {
		if (d.vul === 'NS') { d.vul = 'EW'; }
		else if (d.vul === 'EW') { d.vul = 'NS'; }
	}
	
	if (d.dd) {
		// Rotate double dummy table. (dd.tr order is NSEW)
		let tr = d.dd.tr;
		let ndd = tr.slice(0, 5);
		let sdd = tr.slice(5, 10);
		let edd = tr.slice(10, 15);
		let wdd = tr.slice(15, 20);
		if (nseats === 1) {
			tr = wdd + edd + ndd + sdd;
		}
		else if (nseats === 2) {
		...
		if (nseats === 1 || nseats === 3) {
			...
			if (d.dd.sEW !== undefined) { d.dd.sEW = -d.dd.sEW; }
		}
		...
		for (let i=0; i<sides.length; i++) {
			...
			if (nseats !== 2) {
				// Swap NS <--> EW
				contracts = contracts.replace( /(NS|EW)/g, 
					(match) => { return match === 'NS' ? 'EW' : 'NS'; } );
			}
...
function bsol(d) {
	...
	let BSOLurl = 'https://dds.bridgewebs.com/bsol2/ddummy.htm' + '?lin=' + encodedLIN +
...
async function doubledummy(d, bCacheOnly, callback, bWaitResolve=false) {
	...
	if (d.hand[0].length !== 16) { return; }
	
	const LINvul2BSOLvul = {'o': 'None', 'n': 'NS', 'e': 'EW', 'b': 'All'};
	const BSOLvulnum = ['None', 'All', 'NS', 'EW'];
	const queryTimeOut = 20000;  // 20 sec
	let startTime;
	
	// 4th hand is known from remaining cards. This is a minimal attempt to
	...
	if (item[ddkey] !== undefined) {
		...
		// In highly contrived situations the cached par calculation could be
		// based on the wrong vulnerability, e.g. renumbered boards loaded into a
		// bidding room or the same board presented multiple times deliberately
		// with different vulnerabilities.
		const vul = (d.vul.length === 1) ? LINvul2BSOLvul[d.vul.toLowerCase()] : d.vul;
		if ( BSOLvulnum[dd.v] !== vul ) {
			console.info('BBO Helper: Existing par information is based on wrong' + 
				'vulnerabilty, have %d, need %s', dd.v, vul);
		}
		else {
			dd.wasCached = true;
	
			// Invoke the caller's callback function for the dummy dummy info.
			if ( callback ) { callback(d, dd); }
			return dd;
		}
	}
	
	if (!bCacheOnly) {
		// Vulnerability should be coming in BSOL format but if it is in the single letter
		// format convert it (Tom Trottier reported an issue that this might address).
		const vul = (d.vul.length === 1) ? LINvul2BSOLvul[d.vul.toLowerCase()] : d.vul;
		...
		url = 'https://dds.bridgewebs.com/cgi-bin/bsol2/ddummy?request=m' + 
			'&dealstr=' + 'S:' + fulldeal + '&vul=' + vul + '&club=bbohelper';
		...
		if ( app.pendingDD.indexOf(fulldeal) !== -1 ) {
			...
			console.info('BBO Helper: Squelching duplicated double dummy request: %s', 
				fulldeal);
			return;
		}
		
		// Prefetch double dummy request will not squelch duplicate requests.
		if ( d.source !== 'prefetch') { app.pendingDD.push(fulldeal); }
		
		console.info('BBO Helper: Querying double dummy result for board %d as %s',
			d.bnum, url);
	...
	function fetchError(err) {
		console.error('BBO Helper: dummy dummy query failed for URL %s due to: %s', url, err);
		app.pendingDD.splice(app.pendingDD.indexOf(fulldeal));
	}

	async function fetchSuccess(response) {
		
		const queryTimeStr = ( (Date.now() - startTime) / 1000).toFixed(3);
		...
		// So we need tp convert it. Response includes 20 character 5x4 double dummy
		...
		console.info('BBO Helper: doubledummy(): Double dummy response received from BSOL in %s sec', 
			queryTimeStr);

		// Make sure response is okay.
		if ( response.status !== 200) {
			// Server did not return an HTTP Status code. Request probably timed out.
			console.error('BBO Helper: HTTP status code %d from BSOL ' + 
				'after (%s sec)', response.status, queryTimeStr);
			return;
		}
		
		try { ob = await response.json(); }
		catch (e) {
			console.error('BBO Helper: BSOL response was not valid JSON:', xhr.response); 
			return;
		}
		if (typeof(ob) !== 'object' || ob.sess === undefined ) {
			console.error('BBO Helper: BSOL response lacks expected "sess" attribute:', ob);
			return;
		}
		if (ob.sess.errmsg) {
			// BSOL reported a problem (if everything is okay, then no ERRMSG key) 
			console.error(`BBO Helper: BSOL returned error ${ob.sess.errno}, ` +
				`"${ob.sess.errmsg}", when queried with ${url}`);
			return;
		}
		...
		const sNS = parseInt(ob.scoreNS.slice(3));
		const sEW = parseInt(ob.scoreEW.slice(3));
		
		// Include timestamp for purging older items from cache (code later).
		dd = {"tr": ob.sess.ddtricks, "v": parseInt(ob.vul), "d": Date.now(),
				"cNS": ob.contractsNS.slice(3), "sNS": sNS };
		if (sNS + sEW !== 0) {
			dd['sEW'] = sEW;
			dd['cEW'] = ob.contractsEW.slice(3);
		}		
...
async function getDealTiming(hand, handle) {
	...
	for (i=0; i<keys.length; i++) {
		if ( items[ keys[i] ] === undefined ) { continue; }
		key = keys[i];
		break;
	}
	
	if (key === undefined) {
		console.info('BBO Helper: getDealTiming(): no timing data found in local storage');
		return;
	}
	else {
		console.info('BBO Helper: getDealTiming(): timing data loaded using key:', key);
	}
	...
	if (formatVersion !== 1) { return undefined; }
	...
	for (let i=0; i<t.length-1; i++) {
		...
		if (u16 > 0xd800 && u16 !== 0xffff) { u16 -= 0x800; }
...
async function showCopyAid(d, source) {
	...
	if (source !== 'bbo' && source !== 'handviewer') {
		console.error("BBO Helper: showCopyAid(): source must be 'bbo' or 'handviewer'.");
		return;
	}
	...
	if (dv !== null) { dv.remove(); }  // Doing a refresh
	
	// Create the copy-and-paste-aid.
	dv = document.createElement('div');
	dv.setAttribute('id', 'bh-board-copy-aid');
	
	const isMac = window.navigator.platform && window.navigator.platform.slice(0,3) === 'Mac';

	// Create the button bar.
	const buttonBar = document.createElement('div');
	dv.insertBefore(buttonBar, dv.children[0]);
	buttonBar.style = 'border-bottom: 1px dotted #777; padding-bottom: 0.2em; ' + 
		'text-align: center';
	const modifierKey = isMac ? '&#x2318;' : 'Ctrl';
	let text = browser.i18n.getMessage('copy_aid') + '<br>';
	text += isChrome ?	browser.i18n.getMessage('copy_aid_instructions_Chrome') :
		browser.i18n.getMessage('copy_aid_instructions_Firefox') + ' ' + modifierKey + '+C';
		
	let style = 'float: left; vertical-align: middle; text-align: left';
	buttonBar.innerHTML = `<span style="${style}">` + 
	...
	let imgButton = ['rotate-left-32.png', 'swap-32.png', 'rotate-right-32.png',
		'star-32.png', 'select-32.png', 'HTML-32.png'];
	...
	for (let i=0; i<imgButton.length; i++) {
		...
		if ( rot90[i] !== undefined ) { 
			bt.addEventListener("click", () => { rotate(rot90[i]); }, false);
		}
	...
	imgClose.addEventListener("click", () => { dv.remove(); }, false);
	buttonBar.appendChild(imgClose);
		
	dv.style = 'position: fixed; padding: 0.2em 0.5em 0.2em 0.5em; ' + 
	...
	// Center the copy-aid.
	let el = source === 'handviewer' ? document.getElementById('theDiv') :
	...
	async function rotate(nseats) {
		...
		if ( document.getElementById('bh-board-copy-aid') === null) { return; }
	...
	async function dealerSouth() {
		...
		if (d.contractLevel < 0) { return; }
		if (d.declarer === 'S') { return; }
	...
	async function toggleSelection() {
		...
		if (sel.anchorNode === dv) { sel.removeAllRanges(); return; }
		...
		const msg = browser.i18n.getMessage('board_selected') + ' ' + modifierKey + '+C ' + 
...
function docmessage(message, source, color) {
	...
	const el = source === 'handviewer' ? document.getElementById('theDiv') :
		document.getElementById('bbo_everything');
...
async function exportstorage() {		
	...
	let datestr = date.getFullYear() + '-' + zeroPadInt(date.getMonth()+1,2) +
...
function selectfile(callback) {
	...
	dv.style = 'position: absolute; padding: 0.5em 0.5em 0.5em 0.5em; ' +
	...
	bt.innerHTML = '<span style="font-size: 200%; font-family: sans-serif">Cancel</span>';
...
async function importstorage(file) {
	...
	let ob;
	try { ob = JSON.parse(j); }
	catch(e) { console.error('BBO Helper: ', e.message); return; }
	
	if (typeof(ob) !== 'object') {
		console.error('BBO Helper: importstorage(): JSON did not represent an object');
		return;
	}
	
	let nDoubleDummy = 0, nTiming = 0;
	Object.keys(ob).forEach( function(key) {
		if ( key.startsWith('dd') ) {
			// Not a comprehensive validation.
			if (typeof(ob[key]) === 'object') { nDoubleDummy++; }
			else {
				console.warn('BBO Helper: importstorage(): key', key, 'value is not an object.');
			}
		}
		if ( key.startsWith('tm') ) {
			// Not a comprehensive validation.
			if (typeof(ob[key]) === 'object') { nTiming++; }
			else {
				console.warn('BBO Helper: importstorage(): key', key, 'value is not an object.');
			}
		}
	...
	browser.storage.local.set(ob).then( () => {
		console.info('BBO Helper:', nDoubleDummy, 'double dummy and', nTiming, 
